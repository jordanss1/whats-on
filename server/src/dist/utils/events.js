...
exports.groupEventsById = groupEventsById;
function formatEvents(events, eventFilter = null) {
...
function groupEventsById(events, eventFilter) {
    ...
    let returnedEvents = events.reduce((acc, event) => {
        ...
        let existingKey = ids.find((id) => {
            const bucket = acc[id];
            if (!bucket)
                return false;
            return bucket.dates.some((d) => Math.abs(d.getTime() - eventTime) <= TWO_DAY_MS);
        });
        ...
        if (!existingKey) {
            existingKey =
                Object.keys(acc).find((key) => {
                    ...
                    // must share keywords
                    if (bucket?.keywords !== event.keywords)
                        return false;
        ...
        const isDuplicate = bucket.dates.some((d) => d.getFullYear() === newDate.getFullYear() &&
    ...
    uniqueBuckets
        ...
        if (event.dates.length === 1) {
            ...
            const dayParts = event.dates.map((d) => d.toLocaleDateString('en-GB', {
                weekday: 'short',
                day: 'numeric',
            }));
            // join each one like "Wed 25, Thu 26 & Fri 27" except for the last one which we '&d'
            const daysString = dayParts.length === 2
                ? dayParts.join(' & ')
